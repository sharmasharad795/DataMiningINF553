...
if case_number == 1:
    ...
    df1=df1.filter(lambda x: x != header).map(lambda x: (str(x.split(',')[0]), str(x.split(',')[1]))).groupByKey().map(lambda x: (x[0],list(set(x[1])))).persist()
    ...
    def freq_generator(k,partition_,threshold,frequent_itemsets,previous):
        ...
        for x in candidates:
            x=tuple(sorted(x))
            for y in partition_:
                if set(x).issubset(y[1]) and x not in frequents:
                    frequents[x]=1
                elif set(x).issubset(y[1]) and x in frequents:
                    frequents[x]+=1
    ...
    def mapper1(iterator):
        ...
        pair=set(itertools.combinations(singletons, 2))
        dic2={}
        pairs=[]
        for x in pair:
        
            for y in partition:
                if set(x).issubset(y[1]) and x not in dic2:
                    dic2[x]=1
                elif set(x).issubset(y[1]) and x in dic2:
                    dic2[x]+=1
    ...
    def mapper2(iterator):
        ...
        for x in map1_final:
            count=0
            for y in partition:
                if type(x) is tuple and set(x).issubset(y[1]):
                        count+=1
                else:
                    if x in y[1]:
                        count+=1
    ...
    df1=df1.filter(lambda x: x != header).map(lambda x: (str(x.split(',')[1]), str(x.split(',')[0]))).groupByKey().map(lambda x: (x[0],list(set(x[1])))).persist()
